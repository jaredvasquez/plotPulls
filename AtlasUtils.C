...
void myLineText(Double_t x, Double_t y, Double_t linesize, Int_t lstyle, Int_t mcolor, const char *text, Int_t lwidth = 3)
{
  ...
  Double_t x1=x2-linesize;
  //printf("x1= %f x2= %f y1= %f y2= %f \n",x1,x2,y1,y2);

  TLine mline;
  mline.SetLineWidth(lwidth);
...
void myMarkerText(Double_t x,Double_t y,Int_t color,Int_t mstyle, const char *text,Float_t msize) 
{
  ...
  TLine mline;
  double lsize = 0.04;
  mline.DrawLineNDC( x-(0.4*tsize)-lsize, y, x-(0.4*tsize)+lsize, y );
  // Draw error cap
  mline.DrawLineNDC( x-(0.4*tsize)-lsize, y-0.01, x-(0.4*tsize)-lsize, y+0.01 );
  mline.DrawLineNDC( x-(0.4*tsize)+lsize, y-0.01, x-(0.4*tsize)+lsize, y+0.01 );

  TLatex l; l.SetTextAlign(12); //l.SetTextSize(tsize); 
  l.SetNDC();
  l.DrawLatex(x+lsize,y,text);
}
